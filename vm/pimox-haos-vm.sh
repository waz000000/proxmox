...
if (whiptail --backtitle "Warrens scripts" --title "PiMox HAOS VM" --yesno "This will create a New PiMox HAOS VM. Proceed?" 10 58); then
...
function advanced_settings() {
  METHOD="advanced"
  BRANCH=$(whiptail --backtitle "Warrens scripts" --title "HAOS VERSION" --radiolist "Choose Version" --cancel-button Exit-Script 10 58 3 \
    "$STABLE" "Stable" ON \
    "$BETA" "Beta" OFF \
    "$DEV" "Dev" OFF \
    3>&1 1>&2 2>&3)
  ...
  VMID=$(whiptail --backtitle "Warrens scripts" --inputbox "Set Virtual Machine ID" 8 58 "$NEXTID" --title "VIRTUAL MACHINE ID" --cancel-button Exit-Script 3>&1 1>&2 2>&3)
  exitstatus=$?
  if [ -z "$VMID" ]; then
    VMID="$NEXTID"
  ...
  VM_NAME=$(whiptail --backtitle "Warrens scripts" --inputbox "Set Hostname" 8 58 haos"${BRANCH}" --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3)
  exitstatus=$?
  if [ -z "$VM_NAME" ]; then
    HN="haos${BRANCH}"
  ...
  CORE_COUNT=$(whiptail --backtitle "Warrens scripts" --inputbox "Allocate CPU Cores" 8 58 2 --title "CORE COUNT" --cancel-button Exit-Script 3>&1 1>&2 2>&3)
  exitstatus=$?
  ...
  RAM_SIZE=$(whiptail --backtitle "Warrens scripts" --inputbox "Allocate RAM in MiB" 8 58 4096 --title "RAM" --cancel-button Exit-Script 3>&1 1>&2 2>&3)
  exitstatus=$?
  if [ -z "$RAM_SIZE" ]; then
    RAM_SIZE="4096"
  ...
  BRG=$(whiptail --backtitle "Warrens scripts" --inputbox "Set a Bridge" 8 58 vmbr0 --title "BRIDGE" --cancel-button Exit-Script 3>&1 1>&2 2>&3)
  exitstatus=$?
  if [ -z "$BRG" ]; then
    BRG="vmbr0"
  ...
  MAC1=$(whiptail --backtitle "Warrens scripts" --inputbox "Set a MAC Address" 8 58 "$GEN_MAC" --title "MAC ADDRESS" --cancel-button Exit-Script 3>&1 1>&2 2>&3)
  exitstatus=$?
  if [ -z "$MAC1" ]; then
    MAC="$GEN_MAC"
  ...
  VLAN1=$(whiptail --backtitle "Warrens scripts" --inputbox "Set a Vlan(leave blank for default)" 8 58 --title "VLAN" --cancel-button Exit-Script 3>&1 1>&2 2>&3)
  exitstatus=$?
  if [ $exitstatus = 0 ]; then
    if [ -z "$VLAN1" ]; then
      VLAN1="Default" VLAN=""
  ...
  MTU1=$(whiptail --backtitle "Warrens scripts" --inputbox "Set Interface MTU Size (leave blank for default)" 8 58 --title "MTU SIZE" --cancel-button Exit-Script 3>&1 1>&2 2>&3)
  exitstatus=$?
  if [ $exitstatus = 0 ]; then
    if [ -z "$MTU1" ]; then
      MTU1="Default" MTU=""
