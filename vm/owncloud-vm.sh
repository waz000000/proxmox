...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Warrens scripts" --inputbox "Set Hostname" 8 58 turnkey-owncloud-vm --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z "$VM_NAME" ]; then
      HN="turnkey-owncloud-vm"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
